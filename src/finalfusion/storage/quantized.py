...
from typing import Tuple, Optional, Union, BinaryIO, Iterator, Sequence, List, cast

import numpy as np

from finalfusion.io import _pad_float32, ChunkIdentifier, TypeId, FinalfusionFormatError, find_chunk, \
    _read_required_binary, _write_binary, _serialize_array_as_le
from finalfusion.storage.storage import Storage


class PQ:
    _quantizers: np.ndarray
    _projection: Optional[np.ndarray]
    _reconstructed_len: int
    """
    Product Quantizer
...
class QuantizedArray(Storage):
    ...
    """
    def __new__(cls, pq: PQ, quantized_embeddings: np.ndarray,
        ...
        """
        cls._quantizer = pq
        cls._quantized_embeddings = quantized_embeddings
        cls._norms = norms
        return cls
    ...
    def __array__(self) -> np.ndarray:
        if self._norms is None:
            return self._quantizer.reconstruct(
